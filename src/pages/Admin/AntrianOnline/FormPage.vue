  ...
  <q-page class="q-pa-md">
    <q-card flat bordered class="rounded-2xl max-w-3xl mx-auto">
      <q-card-section class="bg-slate-50 border-b border-slate-100 flex items-center justify-between">
        <div class="text-subtitle1 font-extrabold text-slate-800">Form Pengelolaan Layanan Antrian Online</div>
        <q-btn flat round dense icon="arrow_back" @click="router.back()" />
      </q-card-section>

      <q-card-section class="q-pa-lg">
        <q-form class="q-gutter-y-md" @submit.prevent="save">
          <q-input v-model="form.nama" outlined dense label="Nama Layanan Antrian *" bg-color="white" />
          <q-input v-model="form.slug" outlined dense label="Slug URL *" bg-color="white" />
          <q-input v-model="form.content" outlined type="textarea" rows="5" label="Deskripsi / Ketentuan Layanan *" bg-color="white" />
...
function save() {
  ...
  router.push('/admin/antrian-online')
}
</script>
