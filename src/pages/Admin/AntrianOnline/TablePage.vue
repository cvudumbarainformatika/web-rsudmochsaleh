  ...
  <q-card flat bordered class="rounded-2xl">
    <q-card-section class="bg-slate-50 border-b border-slate-100">
      <div class="flex items-center justify-between">
        <div class="text-subtitle1 font-extrabold text-slate-800">Daftar Pendaftar &amp; Tiket Antrian Online</div>
        <q-input v-model="search" dense outlined rounded placeholder="Cari antrian..." bg-color="white">
...
function editItem(item) {
  router.push(`/admin/antrian-online/form/${item.id}`)
}

function deleteItem(idx) {
  if (confirm('Hapus data layanan antrian ini?')) {
