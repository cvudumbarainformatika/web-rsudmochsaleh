  ...
  actions: {
    ...
    async getDataTable () {
      try {
        ...
        const resp = await api.get('/v1/users', params)
        console.log('items user', resp)
        if (resp.status === 200) {
    ...
    async deletesData (payload) {
      ...
      try {
        ...
        console.log('err user', error.response)
    ...
    async updateStatus (id, sta) {
      ...
      try {
        await api.post('/v1/user/update_status', params).then(resp => {
          notifSuccess(resp)
          this.getDataTable()
        })
      } catch (error) {
