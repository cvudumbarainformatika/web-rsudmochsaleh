...
export const usePpidTable = defineStore('ppid_table', {
  state: () => ({
    items: [],
    meta: {},
    item: {},
  ...
  actions: {
    ...
    async getDataTable () {
      try {
        ...
        const resp = await api.get('/v1/ppids', params)
    ...
    async deletesData (payload) {
      ...
      try {
        await api.post('/v1/ppid/destroy', params).then(resp => {
          notifSuccess(resp)
          this.getDataTable()
        })
      } catch (error) {
        console.log('err ppid', error.response)
    ...
    async updateStatus (id, sta) {
      ...
      try {
        await api.post('/v1/ppid/update_status', params).then(resp => {
          notifSuccess(resp)
          this.getDataTable()
        })
      } catch (error) {
